...
async function syncTravelCompany() {
  const companyId = 'd2c3297a-e40a-4096-8fcf-b7170a44832c';
  console.log('Syncing for travel company:', companyId);

  // 1. Delete legacy 1000, 2000, 3000, 4000, 5000 if no journalLines
  const legacy = await prisma.account.findMany({
    where: {
      companyId,
      code: { in: ['1000', '2000', '3000', '4000', '5000'] }
    },
    include: { journalLines: true, children: true }
  });
  for (const leg of legacy) {
    if (leg.journalLines.length === 0 && leg.children.length === 0) {
      await prisma.account.delete({ where: { id: leg.id } });
      console.log(`Deleted legacy [${leg.code}]`);
    }
  }

  // 2. Upsert all accounts
  const codeToIdMap = new Map();
  for (const acc of FULL_IRAQI_UNIFIED_ACCOUNTS) {
    ...
    codeToIdMap.set(acc.code, res.id);
  }

  // 3. Link parentIds accurately
  for (const acc of FULL_IRAQI_UNIFIED_ACCOUNTS) {
    const accId = codeToIdMap.get(acc.code);
  ...
  console.log('✅ ALL IRAQI UNIFIED ACCOUNTS SYNCED FOR TRAVEL COMPANY!');
}

syncTravelCompany().catch(console.error).finally(() => prisma.$disconnect());
