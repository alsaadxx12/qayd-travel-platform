...
async function syncAll() {
  ...
  for (const company of companies) {
    ...
    // 1. Delete legacy 1000, 2000, 3000, 4000, 5000, 26* if no journal lines
    ...
    for (const leg of legacy) {
      if (leg.journalLines.length === 0 && leg.children.length === 0) {
        ...
        console.log(`Cleaned up unused legacy account [${leg.code}] ${leg.nameAr}`);
      }
    }

    // 2. Upsert all Iraqi Unified accounts
    ...
    console.log(`✅ Iraqi Unified Tree synced and linked for ${company.name}!`);
  }
}
