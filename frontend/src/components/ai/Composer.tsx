  ...
  return (
    <div className="border-t border-slate-100 p-2">
      {(attachedName || attachedPreview) && (
        <div className="flex items-center gap-2 text-[11px] text-slate-600 px-2 pb-1.5">
          {attachedPreview?.startsWith('data:image/') && (
            <img
              src={attachedPreview}
              alt={attachedName || (isArabic ? 'الصورة المرفقة' : 'Attached image')}
              width={40}
              height={40}
              decoding="async"
          ...
          <button
            ...
            aria-label={isArabic ? 'إزالة المرفق' : 'Remove attachment'}
          >
            <IconX size={12} />
          </button>
        </div>
      ...
      <div className="flex items-end gap-1 bg-slate-50 rounded-2xl px-2 py-1.5 border border-slate-200 focus-within:border-[#F45A0A]">
        ...
        <textarea
          aria-label={isArabic ? 'اكتب سؤالك للمستشار' : 'Type your question'}
          value={value}
        ...
        <input
          ref={fileRef}
          type="file"
          aria-label={isArabic ? 'إرفاق ملف' : 'Attach a file'}
          className="hidden"
